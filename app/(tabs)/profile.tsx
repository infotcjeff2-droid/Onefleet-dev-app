...
import { LogOut, Shield, Settings, ChevronRight, User, Mail, Award, Truck, Plus, X, Users, Globe, Image as ImageIcon, Upload } from 'lucide-react-native';
...
function AddUserModal({ visible, onClose, onAdded }: { visible: boolean; onClose: () => void; onAdded: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    if (result.success) {
      ...
      onAdded();
      onClose();
      Alert.alert(t('common.success'), `${t('profile.' + role)} ${t('common.add').toLowerCase()}${t('common.success').toLowerCase() !== 'success' ? '' : ' - ' + t('common.success')}`);
    } else {
      setLoading(false);
...
export default function ProfileScreen() {
  ...
  const handleDeleteUser = (userId: string, userName: string) => {
    Alert.alert(t('common.delete') + ' ' + t('profile.userManagement'), `${t('common.delete')} ${userName}?`, [
      { text: t('common.cancel'), style: 'cancel' },
      {
        text: t('common.delete'),
        style: 'destructive',
        onPress: async () => {
          await useUserManagementStore.getState().deleteUser(userId);
                  ...
                  <View key={managedUser.id}>
                    <View style={styles.userRow}>
                      ...
                      <TouchableOpacity
                        ...
                        onPress={() => handleDeleteUser(managedUser.id, managedUser.name)}
                        hitSlop={12}
                      >
